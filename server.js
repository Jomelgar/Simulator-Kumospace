...
(async () => {
  ...
  server.on("connection", (ws) => {
      ...
      switch (data.type) {
        case "setHive":
          ...
          if(!hive[data.id_hive]){
            ...
            await loadHiveUsers(data.id_hive);
          }
          ...
          if(!found){
            ...
            if(!found.currentLocation){
              const privateRoom = await getPrivateRoomOfUser(ws.hiveID, ws.currentUserID);
              found.currentLocation = privateRoom.id_private_room;
    ...
    ws.on("close", async () => {
      ...
      if(ws.currentUserID && ws.hiveID){
        const user = hive[ws.hiveID].users.find(u => u.id_user === ws.currentUserID);
        if(user){
          user.status = "inactive";
          
          if(user.locationType === "shared"){
            const privateRoom = await getPrivateRoomOfUser(ws.hiveID, ws.currentUserID);
            if(privateRoom){
              user.currentLocation = privateRoom.id_private_room;
              user.locationType = "private";
            }
          } else if(user.locationType === "private"){
            const privateRoom = await getPrivateRoomOfUser(ws.hiveID, ws.currentUserID);
            if(privateRoom && user.currentLocation === privateRoom.id_private_room){
              const usersInPrivateRoom = hive[ws.hiveID].users.filter(
                (u) => u.currentLocation === privateRoom.id_private_room && 
                       u.locationType === "private" && 
                       u.id_user !== user.id_user
              );
              
              for(const guest of usersInPrivateRoom){
                const guestPrivateRoom = await getPrivateRoomOfUser(ws.hiveID, guest.id_user);
                if(guestPrivateRoom){
