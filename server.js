...
async function getRooms(id_hive) {
  ...
  try {
    const priv = await axios.get(
      `http://localhost:3001/api/hive/getPrivateRooms/${id_hive}`
    );
    private_rooms = priv.data;
    const work = await axios.get(
      `http://localhost:3001/api/work_room/getWorkRooms/${id_hive}`
    );
...
function handleEnterWorkspace(userId, workspaceId, type) {
  ...
  if (
    type === "private" &&
    workspace.is_locked &&
    workspace.id_user !== currentUser.id
  ) {
    return;
  }
...
async function handleToggleLock(workspaceId) {
  ...
  if (!wsToToggleLock) {
    ...
    if (wsToToggleLock) {
      await axios.put(
        `http://localhost:3001/api/hive/updatePrivateRoom/${workspaceId}`,
        {
  ...
  await axios.put(
    `http://localhost:3001/api/work_room/updateWorkRoom/${workspaceId}`,
    {
      room_name: wsToToggleLock.room_name,
...
async function handleDeleteSharedWorkspace(userId, workspaceId) {
  ...
  await axios.delete(
    `http://localhost:3001/api/work_room/deleteWorkRoom/${workspaceId}`
  );
}
