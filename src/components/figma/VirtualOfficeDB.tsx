...
import type { Room, User } from "../../pages/Dashboard.tsx";

interface VirtualOfficeDBProps {
  room: Room;
  users: User[];
  onLeave: () => void;
}

export function VirtualOfficeDB({
  room,
  users,
  onLeave,
}: VirtualOfficeDBProps) {
  ...
  const mockMessages = [
    ...
    },
    {
      ...
      message: "Let's start the collaboration",
      time: "10:32 AM",
    },
  ];
              ...
              {users.length < room.maxUsers && (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-center">
                  <div className="text-sm text-zinc-400">
                    {room.maxUsers - users.length} more{" "}
                    {room.maxUsers - users.length === 1 ? "spot" : "spots"}{" "}
                    available
                  </div>
                </div>
              )}
          ...
          {mockMessages.map((msg) => (
            <div key={msg.id} className="flex gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-zinc-700 bg-white border border-zinc-200 px-3 py-2 rounded-lg">
                  {msg.message}
                </p>
              </div>
            </div>
